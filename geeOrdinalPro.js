...
var localization = {
    en: {
        title: "Generalized Estimating Equations, Ordinal",
        navigation: "GEE, Ordinal",
		...
		restrictlabel: "Monotone score parameters when time.exch or RC structure",
		addamountlabel: "Positive constant to add if zero observed counts",
		expparmlabel: "Exponentiate parameter estimates",
        ...
        help: {
            title: "Generalized Estimating Equations, Ordinal",
            r_help: "help(ordLORgee, package ='multgee')",
            body: `
This fits a generalized estimating equations model for an ordinal (>2 category) response variable (ordered categories).
...
The ordinal categorical response variable (required).  Can be numeric, nominal, or character. The categories will be ordered by value where the last category will be the 
highest category for the paramter estimates.<br/><br/>
...
<b>Link function:</b></br>
Specify the link function. "logit" models the log-odds of being less than or equal to each successive increasing outcome level (cumultive logit model), "probit" is for the cumulative probit model,
"cauchit" is for the cumultive cauchit model, "cloglog" is for the cumulative cloglog model, and "acl" is for the adjacent category logit model.  The adjacent category logit model models the log-odds
of each outcome category relative to the category one step higher.  
...
<ul>
	<li>category.exch: category-exchangeable structure.  This assumes a common local odds ratio for each within-cluster pair but allows different pairs to have different odds ratios.</li>
	...
	<li>uniform: uniform structure.  This assumes equal odds ratios for every within-cluster pair and every response category pair (linear by linear association model (Agresti 2013))</li>
...
<b>Monotone score parameters when time.exch or RC structure:</b></br>
Whether monotone score parameters are used</br></br>
...
<b>Exponentiate parameter estimates:</b></br>
Whether to produce a table of exponentiated parameter estimates (e.g. odds ratios for logit and acl link functions). </br></br>
...
If the differences between them are small (say <2 (Touloumis et al. (2013)), this suggests that the uniform structure may be sufficient.
The intrinsic parameters are estimated under the heterogeneous linear-by-linear association model (uniform) (Agresti, 2013).</br></br>
        ...
        var objects = {
		    ...
		    linkfunc: {
                el: new selectVar(config, {
                    no: 'linkfunc',
                    label: localization.en.linkfunclabel,
                    multiple: false,
					width: "w-25",
					style: "mt-4",					
                    extraction: "NoPrefix|UseComma",
			...
			restrict: {
				el: new checkbox(config, {
					label: localization.en.restrictlabel,
					no: "restrict",
					extraction: "Boolean"
				})
			},			
			...
			expparm: {
				el: new checkbox(config, {
					label: localization.en.expparmlabel,
					no: "expparm",
					newline: true,
					extraction: "Boolean"
				})
